...
상태는 `.dev/harness/runs/run-{run_id}/state.json` 에서 관리된다. 세션 포인터는 `.dev/harness/sessions/{session_id}.run_id`.

## 오케스트레이션 체인 순서

1. **interview** — 소크라테스 문답(6개 질문)으로 goal 구체화, `.dev/harness/runs/run-{run_id}/interview.json` 저장
2. **council** — interview.json + goal로 ADR 생성 (product-owner 패널 포함), `.dev/harness/runs/run-{run_id}/adr/` 저장
3. **mini-specify** — goal + ADR로 요구사항 생성, `.dev/harness/runs/run-{run_id}/requirements.json` 저장
4. **taskify** — requirements.json 읽기, 태스크 분해, `.dev/harness/runs/run-{run_id}/spec.json` 저장
...
- PreToolUse 훅: run_id 생성 → `runs/run-{run_id}/state.json` 생성, `sessions/{session_id}.run_id` 기록
...
```
runs/run-{run_id}/state.json: { run_id, skill_name, status: processing|end, goal, paths, timestamp }
...
mini-compound (processing)
  → Stop hook: status=end → delete state.json + session pointer → approve exit
```
...
| 파일 | 생성 주체 | 삭제 주체 | 용도 |
|---|---|---|---|
| `.dev/harness/runs/run-{run_id}/state.json` | `mini-pre-tool-use.sh` | `mini-stop.sh` (mini-compound 완료 후) | run 오케스트레이션 상태 |
...
| `.dev/harness/runs/run-{run_id}/interview.json` | `interview` 스킬 | 유지 | 소크라테스 문답 결과 |
| `.dev/harness/runs/run-{run_id}/requirements.json` | `mini-specify` 스킬 | 유지 | 요구사항 목록 |
| `.dev/harness/runs/run-{run_id}/spec.json` | `taskify` 스킬 | 유지 | 태스크 명세 |
